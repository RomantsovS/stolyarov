...
int main()
{
    ...
    for(i = 0; i < 3; ++i) {
        ...
        ctrl->rb[i] = new Fl_Radio_Round_Button(spacing, y, option_w, option_h, colnames[i]);
    }

    ctrl->cb = new Fl_Check_Button(spacing, 5 * spacing + 3 * option_h, option_w, option_h,
     "show letter");
    ...
    Fl_Button *set_b = new Fl_Button(spacing, buttons_y, button_w, button_h, "Set!");
    set_b->callback(set_callback, (void*)ctrl);

    Fl_Button *close_b = new Fl_Button(2 * spacing + button_w, buttons_y, button_w, button_h, "Quit");
